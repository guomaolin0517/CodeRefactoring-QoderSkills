---
name: P3-controller-custom-common-Controller接口分离
description: "[P3优化] Java微服务Controller层接口分离检查与修复工具。检查并修复Controller是否按外部/内部接口分离原则划分为custom/(外部接口)和common/(内部接口)两级子目录，在其内部按业务功能二级分组，不改变业务逻辑。当用户提到'P3检查'、'P3修复'、'Controller接口分离检查'、'Controller接口分离修复'、'custom/common检查'、'custom/common修复'时使用。"
---

# P3 Controller 层 custom/common 接口分离检查与修复
...
**P3 级别 = Controller 接口分离**：Controller 层按外部/内部接口类型进行一级分组，然后在其中按业务功能进一步分组。不影响功能，属于"结构优化"。

## 使用场景

| 场景 | 触发关键词 | 调用功能 |
|------|-----------|----------|
| 检查 Controller 是否按 custom/common 分离 | "P3检查"、"Controller接口分离检查"、"custom/common检查" | 功能一：接口分离检查 |
| 修复不符合规范的 Controller 分离 | "P3修复"、"Controller接口分离修复"、"custom/common修复" | 功能二：接口分离修复 |
| 先检查再修复 | "P3检查并修复"、"Controller分离全流程" | 功能一 + 功能二 |
...
| 编号 | 检查项 | 严重级别 | 说明 |
|------|--------|---------|------|
| P3-01 | custom/common 一级目录存在性 | FAIL | controller/ 下必须有 custom/ 和 common/ |
| P3-02 | Controller 归属正确性 | FAIL/WARN | 文件不应在 controller/ 根目录或非标准子目录 |
| P3-03 | 二级业务分组合理性 | WARN | 文件超过 10 个应进一步分组 |
| P3-04 | 非 controller 包下的 Controller | FAIL | Controller 类不应在非 controller 包下 |
...
| 文件 | 说明 |
|------|------|
| [scripts/check-rules.md](scripts/check-rules.md) | P3 检查规则清单（4 项详细检查方法与判定标准） |
| [scripts/refactor-rules.md](scripts/refactor-rules.md) | P3 修复规范（迁移策略与执行步骤） |

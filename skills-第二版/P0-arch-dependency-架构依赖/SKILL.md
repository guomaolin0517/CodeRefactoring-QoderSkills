---
name: P0-arch-dependency-架构依赖
description: "[P0严重] Java微服务分层架构依赖检查与修复工具。检查并修复Controller→Controller依赖、Controller直接依赖DAO/Mapper、Controller注入ServiceImpl而非接口、Entity泄露到Controller层、跨模块直接类引用等严重架构违规问题，不改变业务逻辑。当用户提到'P0检查'、'P0修复'、'架构依赖检查'、'架构依赖修复'、'分层依赖检查'、'分层依赖修复'时使用。"
---

...
| Step4 | DAO 层治理 | 重要 | 代码级 | DAO 层目录治理 + model 实体集中到 api 模块（不改变 package） |
...
Step 3: 架构依赖守卫（修复分层依赖违规，依赖 Step 2 完成后的模块结构）
    │
    ▼
Step 4: DAO 层治理（DAO 层目录归位 + model 实体集中到 api 模块，依赖 Step 3 依赖关系已正确）
    │
    ▼
...
**技能全名**：`/Step4-DAO-Model层治理`

**触发关键词**：Step4检查、Step4修复、DAO层治理检查、DAO检查、DAO层治理修复、DAO修复
...
| Step 4 依赖 Step 3 | DAO 层治理需在分层依赖正确后进行，避免迁移引入新的依赖违规 |
| Step 5 依赖 Step 4 | Service 层分离依赖底层 DAO 层结构已规范，model 实体已集中到 api 模块 |
...
| 4 | `/Step4-DAO-Model层治理` | DAO 层治理 + model 实体集中到 api 模块 |

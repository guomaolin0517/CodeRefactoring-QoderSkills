...
| Step3 | DAO 层治理 | 重要 | 代码级 | DAO 层目录治理 + model 实体集中到 api 模块（不改变 package） |
| Step4 | Service 层治理 | 优化 | 代码级 | Service 层 facade/impl 接口实现分离 |
| Step5 | Controller 层治理 | 优化 | 代码级 | Controller 层 custom/common 接口分离 |
| Step6 | 架构依赖守卫 | 严重 | 代码级 | 分层架构依赖检查与修复（在 Step3/4/5 之后执行，确保接口已存在） |
...
Step 2: 工程架构微调（模块重命名，依赖 Step 1 产出的标准结构）
    │
    ▼
Step 3: DAO 层治理（DAO 层目录归位 + model 实体集中到 api 模块，依赖 Step 2 完成后的模块结构）
    │
    ▼
Step 4: Service 层治理（Service 接口/实现分离，补齐缺失的 Service 接口，依赖 Step 3 底层结构已规范）
    │
    ▼
Step 5: Controller 层治理（Controller 接口分离，依赖 Step 4 Service 层已分离）
    │
    ▼
Step 6: 架构依赖守卫（修复分层依赖违规，此时 Service 接口已补齐，可安全修复注入关系）
    │
    ▼
Step 7: 接口与命名规范（命名/路径/Bean 规范，依赖 Step 6 依赖关系已修复）
    │
    ▼
...
**产出结构**（Step 1 完成后，Step 2 执行前）：
```
{module}-module/
├── pom.xml
├── grp-common-{module}/                  # 公共层
├── grp-capability-{module}/              # 能力层容器
│   ├── grp-{module}-model/                 # Model 定义
│   ├── {module}-server/                  # Controller 层（Step 2 将重命名为 {module}-controller）
│   └── {module}-server-com/              # Service 业务实现层（Step 2 将重命名为 {module}-service）
...
**技能全名**：`/Step3-DAO-Model层治理`

**触发关键词**：Step3检查、Step3修复、DAO层治理检查、DAO检查、DAO层治理修复、DAO修复

**优先级**：重要 -- DAO 层目录结构与 model 实体集中管理
...
- **将 model 实体类集中到 `grp-{module}-model` 模块**（不改变 package 声明和引用路径）
...
| S2-03 | DAO 层 mapper/entity 分离 | FAIL |
| S2-04 | 核心四层目录完整性 | WARN |
| S2-05 | resources/mapper 目录对应 | WARN |
| S2-06 | model 实体集中到 model 模块 | INFO |

**重要说明**：
- **移动 model 实体到 model 模块**：物理文件移动到 `grp-{module}-model`，但**保持 package 声明不变**
- **不修改 import 语句**：因 package 未变，所有引用 model 的 import 无需修改
- 目的：便于外部系统统一引用 model 实体，避免散落多个模块
...
**技能全名**：`/Step4-Service层治理`

**触发关键词**：Step4检查、Step4修复、Service层治理检查、Service层检查、Service层治理修复、Service层修复
...
### Step 5: Controller 层治理

**技能全名**：`/Step5-Controller层治理`

**触发关键词**：Step5检查、Step5修复、Controller层治理检查、Controller接口分离检查、Controller层治理修复、custom/common检查
...
**触发关键词**：Step6检查、Step6修复、架构依赖检查、架构依赖修复、分层依赖检查、分层依赖修复

**优先级**：严重 -- 违反分层架构核心依赖规则

**核心职责**：
- 检查并修复 Controller -> Controller 直接依赖（同层耦合，排除 Feign 客户端）
...
**重要说明**：
- Step6 必须在 Step3/4/5 之后执行，因为此时 Service 接口已补齐，依赖修复才能成功
- Feign 客户端（@FeignClient）是合法的跨服务远程调用，不判定为违规
...
| Step 3 依赖 Step 2 | DAO 层治理需在模块命名正确后执行 |
| Step 4 依赖 Step 3 | Service 层分离依赖底层 DAO 层结构已规范，model 实体已集中 |
| Step 5 依赖 Step 4 | Controller 接口分离在 Service 层已分离后细化 |
| Step 6 依赖 Step 3/4/5 | 架构依赖守卫需在 Service 接口已补齐、DAO 目录已规范后执行，确保修复可成功 |
| Step 7 依赖 Step 6 | 命名规范在依赖关系已修复后执行 |
...
| 序号 | 调用命令 | 简要说明 |
|------|---------|--------|
| 1 | `/Step1-工程重构` | 四层架构工程重构 |
| 2 | `/Step2-工程架构微调` | 能力层模块重命名 |
| 3 | `/Step3-DAO-Model层治理` | DAO 层治理 + model 实体集中到 api 模块 |
| 4 | `/Step4-Service层治理` | Service 层治理 |
| 5 | `/Step5-Controller层治理` | Controller 层治理 |
| 6 | `/Step6-架构依赖守卫` | 分层依赖检查与修复 |
| 7 | `/Step7-接口与命名规范` | 接口与命名规范 |
| 8 | `/Step8-公共模块提取` | 公共代码提取到 common 模块 |
| 9 | `/Step9-工程代码优化检查` | 代码质量优化（SQL注入/日志/冗余） |
...
| v4.0.0 | 2026-04-10 | 调整 Step4 为 DAO 层治理（不改变 model package），model 实体集中到 api 模块 |
| v5.0.0 | 2026-04-20 | 优化执行顺序：Step3(DAO)→Step4(Service)→Step5(Controller)→Step6(架构依赖守卫)，架构依赖守卫移至Step6确保Service接口已补齐；统一简化目录命名格式 |

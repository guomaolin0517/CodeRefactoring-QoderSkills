...
description: "[Step6] Java微服务Controller层接口分离检查与修复工具（产品线适配）。根据工程pom.xml中的groupId自动选择对应产品线的检查与修复规则。检查并修复Controller是否按外部/内部接口分离原则划分为custom/(外部接口)和common/(内部接口)两级子目录，不改变业务逻辑。执行链路：Step1→Step2→Step3→Step4→Step5→Step6（当前）→Step7→Step8→Step9。当用户提到'Step6检查'、'Step6修复'、'Controller层治理检查'、'Controller层治理修复'、'custom/common检查'时使用。"
---
...
**执行链路定位**：S4 前置依赖 S3（Service层治理/Step5），后续为 S5（命名规范/Step7）。

...
```
0. 被注入的类是 Feign 客户端？（满足任一条件即判定为 Feign 客户端）
   - 文件路径包含 feign-api/ 或 feignclient/
   - 类上标注了 @FeignClient 注解
   - 类名以 Feign/Fegin/Client 结尾
   - 接口继承了其他 Controller 接口（interface IXxx extends IYyyController）
   - 接口方法上有 @RequestMapping/@GetMapping 等映射注解
   - 包路径包含 .feignclient.
   → SKIP，标注为"Feign 客户端远程调用，合法依赖"，在报告中标记为 SKIP

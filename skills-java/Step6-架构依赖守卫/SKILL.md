---
name: Step6-架构依赖守卫
description: "[Step6] Java微服务分层架构依赖检查与修复工具（产品线适配）。根据工程pom.xml中的groupId自动选择对应产品线的检查与修复规则。检查并修复Controller→Controller依赖、Controller直接依赖DAO/Mapper、Controller注入ServiceImpl而非接口、Entity泄露到Controller层、跨模块直接类引用等严重架构违规问题，不改变业务逻辑。执行链路：Step1→Step2→Step3→Step4→Step5→Step6（当前）→Step7→Step8→Step9。当用户提到'Step6检查'、'Step6修复'、'架构依赖检查'、'架构依赖修复'、'分层依赖检查'、'分层依赖修复'时使用。"
---
...
**执行链路定位**：S1 是代码级重构的关键步骤（依赖关系修复），**必须在 Step3/4/5 目录结构调整之后执行**。此时 Service 接口已补齐、DAO 目录已规范、Controller 已分离，依赖修复可顺利进行。后续按 Step7（命名规范）→ Step8（公共模块提取）执行。

**执行顺序**：
```
Step1 (工程重构) → Step2 (模块重命名)
    ↓
Step3 (DAO/Model 治理) ← 目录结构标准化
    ↓
Step4 (Service 治理) ← 创建缺失接口，facade/impl 分离
    ↓
Step5 (Controller 治理) ← custom/common 分离
    ↓
Step6 (架构依赖守卫) ← 修复依赖注入（此时接口都已存在）✅
    ↓
Step7 (命名规范) ← 统一类名/方法名/路径名
    ↓
Step8 (公共模块提取) ← 提取 util/cache/constant 等
```

**为什么 Step6（架构依赖守卫）在 Step3/4/5 之后？**
- Step4 补齐了所有 Service 接口 → Step6 可将 Controller 中的 ServiceImpl 改为注入接口
- Step5 完成了 Controller 目录分离 → Step6 扫描路径更准确
- 避免了"接口不存在导致修复失败"的问题

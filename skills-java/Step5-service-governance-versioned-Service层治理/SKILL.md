...
description: "[Step5] Java微服务Service层治理检查与修复工具（产品线适配）。根据工程pom.xml中的groupId自动选择对应产品线的检查与修复规则。检查并修复Service层接口/实现分离(facade/impl)、facade目录存在性、Service接口归属正确性、Service实现归属正确性、非Service文件处理等问题，不改变业务逻辑。执行链路：Step1→Step2→Step3→Step4→Step5（当前）→Step6→Step7→Step8→Step9。当用户提到'Step5检查'、'Step5修复'、'Service层治理检查'、'Service层治理修复'时使用。"
---
...
**执行链路定位**：S3 是代码级重构的第三步，前置依赖 S2（DAO-Model层治理），后续为 S4（Controller层治理）。
...
```
Step5-service-governance-versioned-Service层治理/
├── SKILL.md
├── examples/
│   ├── check-report.md
│   └── migration-flow.md

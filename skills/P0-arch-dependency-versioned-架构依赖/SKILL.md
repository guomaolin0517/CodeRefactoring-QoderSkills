---
name: P0-arch-dependency-versioned-架构依赖
description: "[P0严重] Java微服务分层架构依赖检查与修复工具（多版本适配）。根据工程pom.xml中的version自动选择对应版本的检查与修复规则。检查并修复Controller→Controller依赖、Controller直接依赖DAO/Mapper、Controller注入ServiceImpl而非接口、Entity泄露到Controller层、跨模块直接类引用等严重架构违规问题，不改变业务逻辑。当用户提到'P0检查'、'P0修复'、'架构依赖检查'、'架构依赖修复'、'分层依赖检查'、'分层依赖修复'时使用。"
---
...
本技能是 P0 架构依赖检查与修复的**多版本适配版本**。不同工程版本的分层架构规范可能存在差异，本技能会根据工程 `pom.xml` 中声明的 `<version>` 自动选择对应版本的检查与修复规则执行。

## 支持的版本

| 工程版本 | 规则目录 | 说明 |
|----------|----------|------|
| `3.6.0-SNAPSHOT` | [versions/3.6.0-SNAPSHOT/](versions/3.6.0-SNAPSHOT/) | 基线版本，完整 P0 架构依赖检查与修复规则 |
...
| 场景 | 触发关键词 | 调用功能 |
|------|-----------|----------|
| 检查代码是否存在分层依赖违规 | "P0检查"、"架构依赖检查"、"分层依赖检查" | 版本检测 → 依赖检查 |
| 修复已发现的分层依赖违规 | "P0修复"、"架构依赖修复"、"分层依赖修复" | 版本检测 → 依赖修复 |
| 先检查再修复 | "P0检查并修复"、"架构依赖全流程" | 版本检测 → 检查 + 修复 |
...
```
P0-arch-dependency-versioned-架构依赖/

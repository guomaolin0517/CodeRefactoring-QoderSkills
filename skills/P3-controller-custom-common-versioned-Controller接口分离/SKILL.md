---
name: P3-controller-custom-common-versioned-Controller接口分离
description: "[P3优化] Java微服务Controller层接口分离检查与修复工具（多版本适配）。根据工程pom.xml中的version自动选择对应版本的检查与修复规则。检查并修复Controller是否按外部/内部接口分离原则划分为custom/(外部接口)和common/(内部接口)两级子目录，在其内部按业务功能二级分组，不改变业务逻辑。当用户提到'P3检查'、'P3修复'、'Controller接口分离检查'、'Controller接口分离修复'、'custom/common检查'、'custom/common修复'时使用。"
---

# P3 Controller 层 custom/common 接口分离检查与修复（多版本适配）

## 概述

本技能是 P3 Controller 接口分离检查与修复的**多版本适配版本**。不同工程版本的接口分离规范可能存在差异，本技能会根据工程 `pom.xml` 中声明的 `<version>` 自动选择对应版本的检查与修复规则执行。

## 支持的版本

| 工程版本 | 规则目录 | 说明 |
|----------|----------|------|
| `3.6.0-SNAPSHOT` | [versions/3.6.0-SNAPSHOT/](versions/3.6.0-SNAPSHOT/) | 基线版本，完整 P3 Controller 接口分离检查与修复规则 |
...
→ 加载该版本的 RULES.md 作为执行规则

检测到工程版本: 3.6.2-SNAPSHOT
→ 无精确匹配
→ 模糊匹配 3.6 系列，最新为 3.6.1-SNAPSHOT
→ 加载 versions/3.6.1-SNAPSHOT/RULES.md

检测到工程版本: 4.0.0-SNAPSHOT
→ 无精确匹配，无系列匹配
→ 降级到基线: versions/3.6.0-SNAPSHOT/RULES.md
→ 输出警告: "当前工程版本 4.0.0-SNAPSHOT 无匹配规则，已降级使用 3.6.0-SNAPSHOT 基线规则"
```

### Step 1: 加载版本规则

根据 Step 0 确定的版本目录，读取该目录下的 `RULES.md` 文件，获取该版本的完整检查与修复规则。

**加载路径模板：** `versions/{matched-version}/RULES.md`

### Step 2: 执行检查或修复

按照加载的版本规则执行具体的 Controller 接口分离检查或修复操作。规则文件中包含：
- P3 检查项清单（4 项）
- 检查流程
- 修复流程与策略
- 安全约束
...
| 场景 | 触发关键词 | 调用功能 |
|------|-----------|----------|
| 检查 Controller 是否按 custom/common 分离 | "P3检查"、"Controller接口分离检查"、"custom/common检查" | 版本检测 → 接口分离检查 |
| 修复不符合规范的 Controller 分离 | "P3修复"、"Controller接口分离修复"、"custom/common修复" | 版本检测 → 接口分离修复 |
| 先检查再修复 | "P3检查并修复"、"Controller分离全流程" | 版本检测 → 检查 + 修复 |
...
```
P3-controller-custom-common-versioned-Controller接口分离/

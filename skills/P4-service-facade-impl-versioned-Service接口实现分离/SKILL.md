---
name: P4-service-facade-impl-versioned-Service接口实现分离
description: "[P4优化] Java微服务Service层接口实现分离检查与修复工具（多版本适配）。根据工程pom.xml中的version自动选择对应版本的检查与修复规则。检查并修复Service是否按接口/实现分离原则划分为facade/(接口)和impl/(实现)两级子目录，非Service业务文件保留在原子包，不改变业务逻辑。当用户提到'P4检查'、'P4修复'、'Service接口实现分离检查'、'Service接口实现分离修复'、'facade/impl检查'、'facade/impl修复'时使用。"
---

# P4 Service 层 facade/impl 接口实现分离检查与修复（多版本适配）

## 概述

本技能是 P4 Service 接口实现分离检查与修复的**多版本适配版本**。不同工程版本的接口实现分离规范可能存在差异，本技能会根据工程 `pom.xml` 中声明的 `<version>` 自动选择对应版本的检查与修复规则执行。

## 支持的版本

| 工程版本 | 规则目录 | 说明 |
|----------|----------|------|
| `3.6.0-SNAPSHOT` | [versions/3.6.0-SNAPSHOT/](versions/3.6.0-SNAPSHOT/) | 基线版本，完整 P4 Service 接口实现分离检查与修复规则 |
...
按照加载的版本规则执行具体的 Service 接口实现分离检查或修复操作。规则文件中包含：
- P4 检查项清单（4 项）
- 检查流程
- 修复流程与策略
- 安全约束
...
| 场景 | 触发关键词 | 调用功能 |
|------|-----------|----------|
| 检查 Service 是否按 facade/impl 分离 | "P4检查"、"Service接口实现分离检查"、"facade/impl检查" | 版本检测 → 接口实现分离检查 |
| 修复不符合规范的 Service 分离 | "P4修复"、"Service接口实现分离修复"、"facade/impl修复" | 版本检测 → 接口实现分离修复 |
| 先检查再修复 | "P4检查并修复"、"Service分离全流程" | 版本检测 → 检查 + 修复 |
...
```
P4-service-facade-impl-versioned-Service接口实现分离/
